...
class Ui_MainWindow(QtWidgets.QMainWindow):
    ...
    def setupUi(self, MainWindow):
        ...
        font_id = QtGui.QFontDatabase.addApplicationFont("../Assets/Geologica_Auto-SemiBold.ttf")
        ...
        self.titleFrame.setStyleSheet("QFrame { background-color:#F5F5F5; }")
        ...
        self.displayFrame.setStyleSheet("background-color: rgb(255, 255, 255); border-radius:10px;")
        ...
        # Deep Scan Frame on page1
        # self.deepScanFrame = DeepScanFrame(self.page1)
        # self.deepScanFrame.setGeometry(290, 280, 171, 48)

        # Suspicious process list with checkboxes on page2
        self.suspiciousList = QtWidgets.QListWidget(self.page2)
        self.suspiciousList.setGeometry(30, 40, 300, 300)
        self.suspiciousList.setStyleSheet("border: 2px solid #002B5C; border-radius: 10px;")
        self.suspiciousList.setObjectName("suspiciousList")
        self.suspiciousList.itemChanged.connect(self.handle_item_changed)

        # Frames on page2 (optional, keep your existing frames)
        self.frame = QtWidgets.QFrame(self.page2)
        self.frame.setObjectName("frame")
        self.frame.setGeometry(QtCore.QRect(29, 40, 301, 311))
        ...
        self.frame_2.setGeometry(QtCore.QRect(369, 39, 341, 311))
        ...
        self.navFrame.setStyleSheet("background-color: rgb(255, 255, 255); border-radius:20px;")
        ...
        self.monitorButton.setStyleSheet("QPushButton:hover{ background-color:#F5F5F5; }")
        ...
        self.defendButton.setStyleSheet("QPushButton:hover{ background-color:#F5F5F5; }")
    ...
    def add_suspicious_process(self, process_info):
        ...
        for index in range(self.suspiciousList.count()):
            item = self.suspiciousList.item(index)
            if item.data(Qt.ItemDataRole.UserRole) == pid:
                return  # Already listed

        item_text = f"{name} (PID: {pid})"
        item = QListWidgetItem(item_text)
        item.setFlags(item.flags() | Qt.ItemFlag.ItemIsUserCheckable)
        item.setCheckState(Qt.CheckState.Unchecked)
        item.setData(Qt.ItemDataRole.UserRole, pid)  # Store PID for reference
        self.suspiciousList.addItem(item)
    ...
    def handle_item_changed(self, item):
        if item.checkState() == Qt.CheckState.Checked:
            ...
            reply = QMessageBox.question(self, "Confirm Termination",
                                         f"Do you want to terminate process {item.text()}?",
            ...
            if reply == QMessageBox.StandardButton.Yes:
                try:
                    ...
                    proc.terminate()
                    proc.wait(timeout=3)
                    self.suspiciousList.takeItem(self.suspiciousList.row(item))
                    self.update_logs(f"Terminated process {item.text()}")
...
# Modify ProcessMonitor to include pid in emitted process_info
class ProcessMonitor(QtCore.QThread):
    process_signal = QtCore.pyqtSignal(dict)
